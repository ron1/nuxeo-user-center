...
public class UserProfileImporter {
    ...
    public void doImport(CoreSession session) {
        ...
        // Mainly for tests
        if (dataFileName.startsWith("$TMP")) {
            String tmpDir = Framework.getProperty("nuxeo.import.tmpdir");
            if (tmpDir == null) {
                logError(0, "Property nuxeo.import.tmpdir must be set");
                return;
            }
            dataFileName = dataFileName.replace("$TMP", tmpDir);
        }
    ...
    public void doImport(CoreSession session, CSVReader csvReader,
        ...
        log.info(String.format("Done importing CSV file: %s", dataFileName));
    }
