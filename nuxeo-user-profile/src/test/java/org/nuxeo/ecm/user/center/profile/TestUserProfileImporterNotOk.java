        ...
        for (int i = 1; i < 4; i++) {
            ...
            for (String propertyName : propertyNameList) {
                ...
                if (propertyType instanceof BooleanType) {
                    Assert.assertFalse(username + " " + propertyName 
                            + " property value should be false", (Boolean) propertyValue);
                } else if (propertyType.isListType()) {
                    Assert.assertEquals(username + " " + propertyName 
                            + "property list should be empty", 0, ((String[]) propertyValue).length);
                } else {
                    assertNull(username + " " + propertyName 
                            + " property value should be null", propertyValue);
                }
            }
        }
    }
